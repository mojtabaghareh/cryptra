  ...
  useEffect(() => {
    ...
    const unsubscribe = service.subscribe((newPrices) => {
      ...
      newPrices.forEach((coin) => {
        if (holdings[coin.symbol]) {
          total += holdings[coin.symbol] * coin.price;
        }
      });
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col items-center justify-center relative">
          <div className="w-32 h-32 rounded-full border-8 border-accent/30 flex items-center justify-center relative">
            <div
              className="absolute inset-0 rounded-full border-8 border-transparent border-t-success border-r-warning border-b-accent border-l-danger"
