...
export default function XPLeaderboard(): JSX.Element {
  const { t } = useTranslation();
  const [leaders, setLeaders] = useState<XPLeader[]>([]);
  ...
  const fetchLeaders = useCallback(async (): Promise<void> => {
    try {
      setIsLoading(true);
      setError(null);
      const res = await fetch(`/api/v1/leaderboard/xp?timeframe=${timeframe}&limit=50`);
      if (!res.ok) throw new Error('Failed');
      const data: XPLeader[] = await res.json();
      setLeaders(data);
    } catch (err) {
              ...
              <div className={styles.leaderRow}>
                ...
                <div className={styles.leaderInfo}>
                  ...
                  <span className={styles.leaderMeta}>
                    @{leader.username} · {t('leaderboard.level', { level: leader.level })}
                  </span>
                </div>
                <div className={styles.leaderStats}>
                  <Badge variant='primary' size='xs'>{leader.tier}</Badge>
                  <span className={styles.xpValue}>{formatCompactNumber(leader.currentXP)} XP</span>
                </div>
              </div>
            </Card>
          </li>
