...
import { Card, Skeleton, Badge, Avatar, Button, EmptyState, PriceDisplay } from '@cryptra/ui';
import { useTranslation } from '@cryptra/i18n';
import { formatCompactNumber } from '@cryptra/core';
import styles from './Leaderboard.module.css';

interface TradingLeader {
  rank: number;
  userId: string;
  username: string;
  displayName: string;
...
export default function TradingLeaderboard(): JSX.Element {
  const { t } = useTranslation();
  const [leaders, setLeaders] = useState<TradingLeader[]>([]);
  ...
  const fetchLeaders = useCallback(async (): Promise<void> => {
    try {
      setIsLoading(true);
      setError(null);
      const res = await fetch(`/api/v1/leaderboard/trading?timeframe=${timeframe}&limit=50`);
      if (!res.ok) throw new Error('Failed');
      const data: TradingLeader[] = await res.json();
      setLeaders(data);
    } catch (err) {
              ...
              <div className={styles.leaderRow}>
                ...
                <div className={styles.leaderInfo}>
                  ...
                  <span className={styles.leaderMeta}>
                    @{leader.username} · {leader.totalTrades} {t('leaderboard.trades')}
                  </span>
                </div>
                <div className={styles.leaderStats}>
                  <Badge variant={leader.profitLoss >= 0 ? 'success' : 'error'} size='xs'>
                    {leader.profitLoss >= 0 ? '+' : ''}{formatCompactNumber(leader.profitLoss)} USD
                  </Badge>
                  <span className={styles.winRate}>
                    {leader.winRate.toFixed(1)}% {t('leaderboard.winRate')}
                  </span>
                  <span className={styles.volumeText}>
                    <PriceDisplay value={leader.totalVolume} currency='USD' compact />
                  </span>
                </div>
              </div>
            </Card>
